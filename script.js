...
const store = {
	...
	state: {
		...
		config: {
			...
			shell: 'Random',
			size: '3', // 8" shell (index 3 in ['3"', '4"', '6"', '8"', '12"', '16"'])
			autoLaunch: true, // Always enabled
			finale: true, // Always enabled
...
const greetingMessages = [
	'💟💟 Minh Anh ♥️♥️',
	'💟💟 Tiên Nữ Minh Anh ♥️♥️',
	'Chúc Mọi Người Full Sức Khỏe',
	'Chúc Mừng Năm Mới 2026',
	'💟💟 Minh Anh ♥️♥️',
	'💟💟 Tiên Nữ Minh Anh ♥️♥️',
	'Năm Mới Vui Vẻ',
	'Chúc Mọi Người Full Sức Khỏe',
	'Chúc Mọi Người Full May Mắn',
	'Chúc Mọi Người Full Tiền Tài',
...
// Show greeting text at firework burst location
function showGreetingText(x, y) {
	// Randomly select a greeting message
	const message = greetingMessages[Math.floor(Math.random() * greetingMessages.length)];
	...
	// Position at burst location (convert canvas coordinates to screen coordinates)
	const scaleFactor = scaleFactorSelector();
	const containerW = Math.min(window.innerWidth, MAX_WIDTH);
	const containerH = window.innerHeight <= 420 ? window.innerHeight : Math.min(window.innerHeight, MAX_HEIGHT);
	const screenX = (x * scaleFactor) + (window.innerWidth - stageW * scaleFactor) / 2;
	const screenY = (y * scaleFactor) + (window.innerHeight - stageH * scaleFactor) / 2;
...
function startNewYear() {
	...
	for (let i = 0; i < 10; i++) {
		setTimeout(() => {
			...
			shell.launch(Math.random(), Math.random() * 0.5 + 0.3);
		}, i * 200);
	}
}
...
function init() {
	...
	// Keep simulation paused initially - will start after dots animation
	togglePause(true);
	...
	// Dots animation should already be started in startApp()
	// If dots animation not available, start fireworks
	if (typeof initDotsAnimation !== 'function') {
		togglePause(false);
	}
}
...
function seqSmallBarrage() {
	...
	// For Random mode, prepare a list of unique shell types to ensure
	// that every shell in this barrage looks different.
	let barrageShellFactories = null;
	let barrageShellIndex = 0;
	if (isRandomShell) {
		// Exclude the meta "Random" entry itself.
		const availableNames = shellNames.filter(name => name !== 'Random');
		const usedNames = new Set();
		barrageShellFactories = [];

		// Pick up to `barrageCount` distinct shell types.
		while (barrageShellFactories.length < barrageCount && usedNames.size < availableNames.length) {
			const name = MyMath.randomChoice(availableNames);
			if (!usedNames.has(name)) {
				usedNames.add(name);
				barrageShellFactories.push(shellTypes[name]);
			}
		}
	}
	
	// (cos(x*5Ï€+0.5Ï€)+1)/2 is a custom wave bounded by 0 and 1 used to set varying launch heights
	function getShellFactory() {
		// If user picked a specific shell type, just use that one.
		if (!isRandomShell) {
			return shellTypes[shellNameSelector()];
		}
		// Otherwise, rotate through the prepared list of unique shell types.
		const factory = barrageShellFactories[barrageShellIndex % barrageShellFactories.length];
		barrageShellIndex++;
		return factory;
	}

	function launchShell(x) {
		const shellType = getShellFactory();
...
let isFirstSeq = true;
const finaleCount = 20; // Reduced from 24 for better balance
const finaleAutoFireCount = 3; // Number of Auto Fire sequences between finale bursts
let currentFinaleCount = 0;
let currentAutoFireCount = 0;
function startSequence() {
	...
	if (finaleSelector()) {
		// On the first finale step, fire a horizontal barrage sequence,
		// then fall back to the normal fast random shells logic.
		if (currentFinaleCount === 0) {
			currentFinaleCount++;
			currentAutoFireCount = 0; // Reset auto fire counter
			return seqSmallBarrage();
		}

		// If we've finished the finale burst, insert Auto Fire sequences
		if (currentFinaleCount >= finaleCount) {
			if (currentAutoFireCount < finaleAutoFireCount) {
				currentAutoFireCount++;
				// Random 1-3 shells fired simultaneously
				const shellCount = Math.floor(Math.random() * 3) + 1; // 1, 2, or 3
				const baseSize = shellSizeSelector();
				...
				if (shellCount === 1) {
					...
					const size1 = getRandomShellSize();
					const size2 = getRandomShellSize();
					const size3 = getRandomShellSize();
					const shell1 = new Shell(shellFromConfig(size1.size));
					const shell2 = new Shell(shellFromConfig(size2.size));
					const shell3 = new Shell(shellFromConfig(size3.size));
					const offset1 = Math.random() * 0.15 - 0.075;
					const offset2 = Math.random() * 0.15 - 0.075;
					const offset3 = Math.random() * 0.15 - 0.075;
					shell1.launch(0.25 + offset1, size1.height);
					shell2.launch(0.5 + offset2, size2.height);
					shell3.launch(0.75 + offset3, size3.height);
					return 900 + Math.random() * 600 + Math.max(shell1.starLife, shell2.starLife, shell3.starLife);
				}
			}
			else {
				// After Auto Fire sequences, reset and start new finale cycle
				currentFinaleCount = 0;
				currentAutoFireCount = 0;
				return seqSmallBarrage();
			}
		}

		// Continue with finale fast shells
		seqRandomFastShell();
		currentFinaleCount++;
		return 220; // Increased from 170ms for better visibility
	}
...
function updateGlobals(timeStep, lag) {
	...
	// Countdown timer is now updated via setInterval for realtime accuracy
...
class Shell {
	...
	launch(position, launchHeight) {
		...
		// No sound on launch; only play sounds on explosion.
	}
	
	burst(x, y) {
		// Randomly show greeting text at burst location (50% chance)
		if (Math.random() < 0.5) {
			showGreetingText(x, y);
		}
		...
		if (this.comet) {
			...
			// Use a softer, more crackly explosion sound instead of a sharp \"gun-like\" burst.
			soundManager.playSound('burstSmall', soundScale);
			if (Math.random() < 0.6) {
				soundManager.playSound('crackleSmall', soundScale);
			}
		}
	}
}
...
const soundManager = {
	...
	resumeAll() {
		...
		// Using a small timeout after first interaction is usually enough.
		setTimeout(() => {
			this.ctx.resume();
		}, 250);
	},
...
// Function to start the app after user interaction
function startApp() {
	// Hide audio overlay
	...
	// Start dots animation immediately (no delay)
	if (typeof initDotsAnimation === 'function') {
		initDotsAnimation();
	}
	if (typeof startDotsSequence === 'function') {
		startDotsSequence();
	}
	
	// Play background music (321_join.mp3)
	...
	if (startButton) {
		...
		if (audioOverlay) {
			audioOverlay.addEventListener('click', (e) => {
				if (e.target === audioOverlay || e.target === startButton) {
					startApp();
				}
			});
		}
	} else {
		// Fallback: if overlay doesn't exist, start normally
		setLoadingStatus('Lighting Fuses');
		setTimeout(() => {
			soundManager.preload()
			.then(
...
// Hàm tạo điểm nổ theo hình trái tim
function getHeartPoint(t) {
    // Công thức toán học cho hình trái tim
